...
public class DevourerWandProjectileEntity extends AbstractArrow implements ItemSupplier {
	public static final ItemStack PROJECTILE_ITEM = new ItemStack(RorModItems.RUNE.get());
	private int knockback = 0;

	public DevourerWandProjectileEntity(EntityType<? extends DevourerWandProjectileEntity> type, Level world) {
		super(type, world);
	}

	public DevourerWandProjectileEntity(EntityType<? extends DevourerWandProjectileEntity> type, LivingEntity entity, Level world, @Nullable ItemStack firedFromWeapon) {
	...
	public void onHitEntity(EntityHitResult entityHitResult) {
        ...
        RorMod.queueServerWork(5, () -> {
			if (world instanceof ServerLevel _level) {
				Entity entityToSpawn = EntityType.EVOKER_FANGS.spawn(_level, BlockPos.containing(entity.getX() + 2, entity.getY(), entity.getZ() + 2), MobSpawnType.MOB_SUMMONED);
	...
	public void onHitBlock(BlockHitResult blockHitResult) {
		...
		RorMod.queueServerWork(5, () -> {
			if (world instanceof ServerLevel _level) {
				Entity entityToSpawn = EntityType.EVOKER_FANGS.spawn(_level, BlockPos.containing(x, y + 0.5, z), MobSpawnType.MOB_SUMMONED);
				if (entityToSpawn != null) {
					entityToSpawn.setYRot(world.getRandom().nextFloat() * 360F);
				}
			}
	...
	public static DevourerWandProjectileEntity shoot(Level world, LivingEntity entity, RandomSource source) {
		return shoot(world, entity, source, 2f, 0.1, 0);
	}

	public static DevourerWandProjectileEntity shoot(Level world, LivingEntity entity, RandomSource source, float pullingPower) {
		return shoot(world, entity, source, pullingPower * 2f, 0.1, 0);
	}

	public static DevourerWandProjectileEntity shoot(Level world, LivingEntity entity, RandomSource random, float power, double damage, int knockback) {
		DevourerWandProjectileEntity entityarrow = new DevourerWandProjectileEntity(RorModEntities.DEVOURER_WAND_PROJECTILE.get(), entity, world, null);
	...
	public static DevourerWandProjectileEntity shoot(LivingEntity entity, LivingEntity target) {
		DevourerWandProjectileEntity entityarrow = new DevourerWandProjectileEntity(RorModEntities.DEVOURER_WAND_PROJECTILE.get(), entity, entity.level(), null);
