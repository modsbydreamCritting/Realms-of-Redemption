...
public class JawWandItem extends Item {
	public JawWandItem() {
	...
	public void onUseTick(Level world, LivingEntity entity, ItemStack itemstack, int time) {
		if (!world.isClientSide() && entity instanceof ServerPlayer player) {
			...
			if (player.getAbilities().instabuild || stack != ItemStack.EMPTY) {
				JawWandProjectileEntity projectile = JawWandProjectileEntity.shoot(world, entity, world.getRandom());
	...
	private ItemStack findAmmo(Player player) {
		ItemStack stack = ProjectileWeaponItem.getHeldProjectile(player, e -> e.getItem() == JawWandProjectileEntity.PROJECTILE_ITEM.getItem());
		if (stack == ItemStack.EMPTY) {
			for (int i = 0; i < player.getInventory().items.size(); i++) {
				ItemStack teststack = player.getInventory().items.get(i);
				if (teststack.getItem() == JawWandProjectileEntity.PROJECTILE_ITEM.getItem()) {
					stack = teststack;
					break;
				}
			}
