...
public class LaserEntity extends AbstractArrow implements ItemSupplier {
	public static final ItemStack PROJECTILE_ITEM = new ItemStack(RorModItems.LASER_BALL.get());
	...
	protected ItemStack getDefaultPickupItem() {
		return new ItemStack(RorModItems.LASER_BALL.get());
	}
	...
	public void onHitEntity(EntityHitResult entityHitResult) {
		...
		entityHitResult.getEntity().igniteForSeconds(2);
	}
	...
	public static LaserEntity shoot(Level world, LivingEntity entity, RandomSource random, float power, double damage, int knockback) {
		LaserEntity entityarrow = new LaserEntity(RorModEntities.LASER.get(), entity, world, null);
		...
		world.playSound(null, entity.getX(), entity.getY(), entity.getZ(), BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("ror:laser_shoots")), SoundSource.PLAYERS, 1, 1f / (random.nextFloat() * 0.5f + 1) + (power / 2));
		return entityarrow;
	}

	public static LaserEntity shoot(LivingEntity entity, LivingEntity target) {
		LaserEntity entityarrow = new LaserEntity(RorModEntities.LASER.get(), entity, entity.level(), null);
		...
		entity.level().playSound(null, entity.getX(), entity.getY(), entity.getZ(), BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("ror:laser_shoots")), SoundSource.PLAYERS, 1, 1f / (RandomSource.create().nextFloat() * 0.5f + 1));
		return entityarrow;
	}
}